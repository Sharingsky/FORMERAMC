...
from lib.data import get_dataset,get_split_dataset
...
class ChannelPruningEnv:
    ...
    def __init__(self, model, checkpoint, data, preserve_ratio, args, n_data_worker=4,
        ...
        self.model = model
        import copy
        self.premodel = copy.deepcopy(model)
        self.checkpoint = checkpoint
        self.n_data_worker = n_data_worker
        self.batch_size = batch_size
        self.data_type = data#保存是什么数据集
        ...
        self.channel_round = args.channel_round#让通道是8的整数倍
        self.acc_metric = args.acc_metric
        self.data_root = args.data_root

        self.export_model = export_model
        self.use_new_input = use_new_input#使用新的数据，可能导致过拟合，团队在做的时候用的是固定的一组数据
        ...
        # build indexs
        self._build_index()#遍历模型：记录索引，对应的layer，layer的类型，不剪枝的idx加入buffer，记录初始的通道数
        self.n_prunable_layer = len(self.prunable_idx)#经过build得到

        # extract information for preparing
        self._extract_layer_information()#NCALIBATCH次数的前向，不知道为什么要这么多次，\
        # 对每个batch都forward，但在这里应该没开始剪枝啊
        # build embedding (static part)
        self._build_state_embedding()#生成静态的state字典，初始化动态参数，对embedding做正则化

        # build reward
        self.reset()  # restore weight
        self.org_acc = self._validate(self.val_loader, self.model) #初始的验证准确率
        print('=> original acc: {:.3f}%'.format(self.org_acc))
        self.org_model_size = sum(self.wsize_list)#初始模型参数量
        ...
        self.expected_preserve_computation = self.preserve_ratio * self.org_flops#要达到的剪枝总量
        self.reward = eval(args.reward)#计算reward的方法,eval可以把字符串代表的函数传进去
    ...
    def compact(self,model1,model2):
        fea_in = torch.randn((1,3,32,32)).cuda()
        ou1 = model1(fea_in)
        ou2 = model2(fea_in)
    ...
    def step(self, action):
        ...
        else:#做CLIP，要计算当前剪枝了多少，剩余还要剪多少，确保总体剪枝满足要求
        ...
        if self._is_final_layer():
            ...
            # TODO:比较featuremap
            if self.args.compa:
                self.compact(self.model,self.premodel)
    ...
    def reset(self):#重读存档点，回到第一层，空策略，重置状态embedding
    ...
    def prune_kernel(self, op_idx, preserve_ratio, preserve_idx=None):
        ...
        d_prime = int(np.ceil(d_prime * 1. / self.channel_round) * self.channel_round)#channel_round默认值是8
        if d_prime > c:
        ...
        if preserve_idx is None:  # not provided, generate new
            importance = np.abs(weight).sum((0, 2, 3))#把每个通道分别加起来
        ...
        mask = np.zeros(weight.shape[1], bool)#先置零
        mask[preserve_idx] = True#再在保留的idx上置为true
        # reconstruct, X, Y <= [N, C]
        masked_X = X[:, mask]#X是input_feat
       # TODO:不做reconstruct会怎么样
        if weight.shape[2] == 1:  # 1x1 conv or fc
            from lib.utils import least_square_sklearn
            rec_weight = least_square_sklearn(X=masked_X, Y=Y)#线性回归
        ...
        elif weight.shape[2] == 3:#3X3
            from lib.utils import least_square_sklearn
            k_h, k_w = weight.shape[2], weight.shape[3]
            X_mask = X[:, mask, :].reshape(-1, d_prime * k_h * k_w)
        ...
        if self.export_model:  # prune previous buffer ops
            ...
            for idx in range(prev_idx, op_idx):
                ...
                if type(m) == nn.Conv2d:  # depthwise
                    ...
                    if m.groups == m.in_channels:#else怎么办
    ...
    def _action_wall(self, action):
        ...
        other_comp = 0
        this_comp = 0
        for i, idx in enumerate(self.prunable_idx):#针对可剪枝层，同时计算buffer_layer
            ...
            if i == self.cur_ind - 1:  # TODO: add other member in the set
                this_comp += flop * self.strategy_dict[idx][0]#计算量乘以输入通道的剪枝率
                # add buffer (but not influenced by ratio)
                other_comp += buffer_flop * self.strategy_dict[idx][0]
            elif i == self.cur_ind:
                this_comp += flop * self.strategy_dict[idx][1]#乘以输出通道剪枝率
    ...
    def _init_data(self):
        ...
        if self.use_real_val:  # use the real val set for eval, which is actually wrong
            ...
            self.train_loader, self.val_loader, n_class = get_dataset(self.data_type, self.batch_size,
                                                                            self.n_data_worker,data_root=self.data_root
                                                                            )
        else:
    ...
    def _build_index(self):
        self.prunable_idx = []#剪枝model的索引
        self.prunable_ops = []#剪枝的model
        self.layer_type_dict = {}#layer类型
        self.strategy_dict = {}#初始只赋值左右边界
        self.buffer_dict = {}#如果之前是深度可分离的layer就把它们加到bufferdict
        this_buffer_list = []
        self.org_channels = []#初始的input通道数，如果是leaner就是features
        ...
        for i, m in enumerate(self.model.modules()):
            if type(m) in self.prunable_layer_types:
                if type(m) == nn.Conv2d and m.groups == m.in_channels:  # depth-wise conv, buffer
                    ...
                    self.prunable_idx.append(i)#记录索引，对应的layer，layer的类型，不剪枝的idx加入buffer，记录初始的通道数
        ...
        self.strategy_dict[self.prunable_idx[0]][0] = 1  # modify the input    输入不剪枝
        self.strategy_dict[self.prunable_idx[-1]][1] = 1  # modify the output    输出不剪枝
        ...
        if self.args.model == 'mobilenetv2':  # TODO: to be tested! Share index for residual connection
            ...
            last_ch = -1
            share_group = None
            for c_idx in connected_idx:#看瓶颈有没有变宽或变窄
                if self.prunable_ops[c_idx].in_channels != last_ch:  # new group
                    last_ch = self.prunable_ops[c_idx].in_channels#上一个group的inchannels
    ...
    def _extract_layer_information(self):
        m_list = list(self.model.modules())#模型列表

        self.data_saver = []#保存下数据后面重用
        self.layer_info_dict = dict()#层信息都存在这里
        self.wsize_list = []#权重size
        self.flops_list = []#flops
        ...
        for idx in self.prunable_idx + self.buffer_idx:  # get all#这里新的前向是为了记录flops和params
            m = m_list[idx]
            m.old_forward = m.forward#正常的前向赋给old_forward
        ...
        with torch.no_grad():
            for i_b, (input, target) in enumerate(self.train_loader):  # use image from train set
                if i_b == self.n_calibration_batches:#到了多少batch就结束
                    break
                self.data_saver.append((input.clone(), target.clone()))
                input_var = torch.autograd.Variable(input).cuda()

                # inference and collect stats#old_forward
                _ = self.model(input_var)

                if i_b == 0:  # first batch#第一个batch把所有layer_info写好
                ...
                for idx in self.prunable_idx:#注意这是每个batch都要做一次循环
                    f_in_np = m_list[idx].input_feat.data.cpu().numpy()#layer的输入
                    f_out_np = m_list[idx].output_feat.data.cpu().numpy()#layer的输出
                    if len(f_in_np.shape) == 4:  # conv
                        ...
                        # f_out2save shape is [B*samle_num, C_out]
                            from lib.utils import feature_sample
                        ...
                        else:  # 1x1 conv只有这里是和batch相关的，因为要在不同的图像输入上采样
                            # assert f_out_np.shape[2] == f_in_np.shape[2]  # now support k=3
                            #随机取n_points个点的值,减少运算？
    ...
    def _regenerate_input_feature(self):#感觉用的还是一样的数据，怕训练太多?
        ...
        with torch.no_grad():
            for i_b, (input, target) in enumerate(self.data_saver):#和抽取layer_info时一样的值
                input_var = torch.autograd.Variable(input).cuda()
    ...
    def _build_state_embedding(self):
        # build the static part of the state embedding#静态的可以先加进去
        layer_embedding = []
        module_list = list(self.model.modules())
        for i, ind in enumerate(self.prunable_idx):
            m = module_list[ind]#可剪枝的model列表
            this_state = []
            if type(m) == nn.Conv2d:
                ...
                this_state.append(np.prod(m.weight.size()))  # weight size#累乘
        ...
        # normalize the state#正则化，因为尺寸不一样
    ...
    def _validate(self, val_loader, model, verbose=False):#验证
        '''
        ...
        with torch.no_grad():
            for i, (input, target) in enumerate(val_loader):
                target = target.cuda(non_blocking=True)#非阻塞
                input_var = torch.autograd.Variable(input).cuda()
